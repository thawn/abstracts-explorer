...
def rewrite_and_search_papers(
    ...
    This tool takes a natural language question, rewrites it into an optimized search
    query, and returns relevant research papers from the embeddings database.
    Use this tool when the user asks specific questions about research concepts,
    techniques, methods, or wants detailed information from papers.
    
    Parameters
    ----------
    query : str
        The user's question or query to rewrite and search
    n_results : int, optional
        Number of papers to retrieve (default: 5)
    conferences : list[str], optional
        Filter by specific conferences (e.g., ['NeurIPS', 'ICLR'])
    years : list[int], optional
        Filter by specific years (e.g., [2024, 2025])
    collection_name : str, optional
        Name of ChromaDB collection (uses config default if not provided)
    ...
    -------
    str
        JSON string containing search results with papers
    """
    try:
        ...
        # Initialize embeddings manager and database
        ...
        if conferences:
            metadata_filter["conference"] = {"$in": conferences}
        if years:
            metadata_filter["year"] = {"$in": years}
        
        # The query is already provided - we'll use it directly for search
        # In a more advanced version, we could use an LLM here to rewrite it,
        # but for now we'll keep it simple and rely on semantic search
        logger.info(f"Searching for papers with query: {query}")
        
        # Search for relevant papers
        search_results = em.search_similar(
            query, 
            n_results=n_results, 
        ...
        if not search_results["ids"][0]:
            ...
            # Format papers
            papers = format_search_results(search_results, db, include_documents=True)
        ...
        logger.error(f"Failed to rewrite and search papers: {str(e)}")
        return json.dumps({"error": str(e)}, indent=2)
