...
class ChromaDBPaperMetadata(BaseModel):
    """
    Pydantic model for parsing ChromaDB paper metadata back to proper types.

    ChromaDB stores all metadata values as strings (see
    ``EmbeddingsManager.add_paper``).  This model converts those strings back
    to their canonical Python types so that downstream consumers never need
    ad-hoc parsing.

    All fields are optional because ChromaDB metadata may contain only a
    subset of paper fields.
    ...
    authors : str, optional
        Semicolon-separated author string (kept as-is for DB compatibility)
    ...
    year : int, optional
        Publication year (coerced from string)
    conference : str, optional
        Conference name
    original_id : int, optional
        Paper ID from the original source (coerced from string)
    paper_pdf_url : str, optional
        URL to paper PDF
    poster_image_url : str, optional
        URL to poster image
    ...
    keywords : str, optional
        Comma-separated keywords string (kept as-is for DB compatibility)
    starttime : str, optional
        Start time
    endtime : str, optional
        End time
    ...
    See Also
    --------
    LightweightPaper : Strict validation model used during data import.
    EmbeddingsManager.parse_chromadb_metadata : Central entry point that
        applies this model to raw ChromaDB metadata dicts.
    """
    ...
    def coerce_year(cls, v: Any) -> Optional[int]:
        """Convert string year to int, returning None for invalid values."""
        if v is None or v == "":
            return None
        try:
            return int(v)
    ...
    @classmethod
    def coerce_original_id(cls, v: Any) -> Optional[int]:
        """Convert string original_id to int, returning None for invalid values."""
        if v is None or v == "":
            return None
        try:
            return int(v)
