...
@app.route("/api/clusters/hierarchy/level/<int:level>", methods=["GET"])
def get_hierarchy_level(level):
    """
    ...
    ----------
    level : int
        Hierarchy level (0 = leaf level, higher = more merged)
    
    Query Parameters
    ---------------
    parent_id : int (optional)
        If provided, only return children of this parent cluster
    ...
    dict
        ...
        - labels: Labels for clusters at this level
    """
    try:
        config = get_config()
        em = get_embeddings_manager()
        ...
        # Get parent_id filter if provided
        parent_id = request.args.get("parent_id", type=int)
        
        # Get current clustering results (should have hierarchy info if agglomerative was used)
        current_model = config.embedding_model
        
        # Try to get from cache first
        cached_results = database.get_clustering_cache(
            embedding_model=current_model,
            reduction_method="tsne",  # Default - should be improved to handle any method
            n_components=2,
            clustering_method="agglomerative",
            n_clusters=None,
        )
        
        if not cached_results or 'cluster_hierarchy' not in cached_results:
            return jsonify({
                "error": "No agglomerative clustering hierarchy available. Please run agglomerative clustering first."
            }), 404
        
        # Reconstruct ClusteringManager to use hierarchy methods
        from abstracts_explorer.clustering import ClusteringManager
        cm = ClusteringManager(em, database)
        cm.load_embeddings()
        
        # We need to re-cluster to get the hierarchy - this is a limitation
        # In production, we'd want to serialize the full hierarchy in cache
        cm.cluster(method='agglomerative', n_clusters=cached_results['statistics']['n_clusters'])
        
        # Get clusters at the requested level
        level_info = cm.get_hierarchy_level_clusters(level)
        
        # If parent_id specified, filter to only children of that parent
        if parent_id is not None:
            tree = cm.cluster_hierarchy['tree']
            if parent_id in tree['nodes']:
                parent_children = tree['nodes'][parent_id]['children']
                level_info['clusters'] = [
                    c for c in level_info['clusters']
                    if c['node_id'] in parent_children
                ]
        
        # Generate labels for this level
        hierarchical_labels = cm.generate_hierarchical_labels(use_llm=True)
        
        # Add labels to cluster info
        for cluster in level_info['clusters']:
            node_id = cluster['node_id']
            cluster['label'] = hierarchical_labels.get(node_id, f"Cluster {node_id}")
        
        return jsonify(level_info)
        
    except Exception as e:
        logger.error(f"Error getting hierarchy level: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
